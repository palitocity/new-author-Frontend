...
const Orders = () => {
  ...
  const deleteAllOrders = async () => {
    try {
      const res = await axios.delete("/order/delete-all", {
        headers: {
          Authorization: `Bearer ${token}`,
        },
        data: {
          confirm: "DELETE_ALL_ORDERS", // if you added confirmation on backend
        },
      });

      if (res.data.success) {
        toast.success("All orders deleted successfully");
        setOrders([]); // clear UI immediately
      }
    } catch (err: any) {
